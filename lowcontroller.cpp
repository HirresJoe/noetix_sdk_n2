...
#ifdef RK3588
unsigned char *LowController::load_model(const char *filename,
                                         int *model_size) {
...
void LowController::dump_tensor_attr(rknn_tensor_attr *attr) {
        ...
        printf(" index = %d ,name= "
               "%s,n_dims=%d,dims=[%s],n_elems=%d,size=%d,w_stride=%d,size_"
               "with_stride=%d,fmt=%s,"
               "type=%s, qnt_type=%s,zp=%d,scale=%f\n",
               attr->index, attr->name, attr->n_dims, shape_str.c_str(),
               attr->n_elems, attr->size, attr->w_stride,
               attr->size_with_stride, get_format_string(attr->fmt),
...
float LowController::as_float(const unsigned int x) { return *(float *)&x; }
...
bool LowController::loadrknnmodel(std::string modle_path) {
        ...
        unsigned char *model_data =
            load_model(policyFilePath.c_str(), &model_data_size);
...
bool LowController::handleWalkMode() {
        ...
        for (int i = 0; i < 8; i++) {
                ...
                scalar_t pos_des;
                // std::array<MotorState,18> joint_state =
                // controllerbase.get_joint_state();
                const std::shared_ptr<const std::array<MotorState, 18>> ms =
                    motor_state_buffer_.GetData();
                if (ms) {
...
void LowController::computeActionsrknn() {
        ...
        // std::cout<<"policyObservations_.size()
        // "<<policyObservations_.size()<< std::endl;
        ...
        // printf("output buf size is %d,n_elems
